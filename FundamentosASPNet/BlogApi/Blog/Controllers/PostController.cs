...
namespace Blog.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAsync([FromServices]BlogDataContext context, [FromQuery]int page = 0, [FromQuery] int pageSize = 25)
        {
            try
            {
                var count = await context.Posts.AsNoTracking().CountAsync(); //facilita para o front
        ...
        public async Task<IActionResult> GetByCategory([FromRoute]string category, [FromServices]BlogDataContext context, [FromQuery] int page = 0, [FromQuery] int pageSize = 25)
        {
            try
            {
                ...
                var posts = await context
                    ...
                    .Include(x => x.Author)
                    .Include(x => x.Category)
                    .Where(w => w.Category.Slug == category)
                    .Select(x => new ListPostsViewModel
                    {
                        Id = x.Id,
                        Title = x.Title,
